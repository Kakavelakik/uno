...
//        if (cpuVisible) cpuVisible = false
        ...
        if (handToUpdate === cpu2Hand) {
            ...
            cardClass = 'cpu2'
//            if (cpu2Visible) cpu2Visible = false
        }
        else {
...
const animateDrawCard = (player) => {
    let playerClass
    if ((player === cpuHand) || (player === cpu2Hand)) playerClass = 'cpu-draw'
...
const chooseColor = (rgb) => {
    ...
    else {setTimeout(playCPU2, cpuDelay)        
    }    
}
...
const skipOrEndTurn = () => {
    ...
    if (playPile[playPile.length - 1].changeTurn) {
        ...
        if (playerTurn==1) { setTimeout(playCPU, cpuDelay)}
            ...
            if (playerTurn==2) { setTimeout(playCPU, cpuDelay)}
//            else {setTimeout(playCPU, cpuDelay)}
        }
...
const updateScores = () => {
    ...
    cpuScoreDom.style.color = 'rgb(0, 140, 0)'
...
//    else cpuScoreDom.style.color = 'rgb(121, 2, 2)'
    ...
    cpu2ScoreDom.style.color = 'rgb(0, 140, 0)'

//    if (cpu2Score < gameOver / 2) cpu2ScoreDom.style.color = 'rgb(0, 140, 0)'
//    else cpu2ScoreDom.style.color = 'rgb(121, 2, 2)'
...
const endGame = () => {
    ...
    if (playerScore > gameOver) {
        loseFX.play()
        gameDom.textContent = 'CPU1 won the game... Play again?'
    }  
    else {
        if (cpuScore > gameOver) {
            loseFX.play()
            gameDom.textContent = 'CPU2 won the game... Play again?'
        }  
        else {
        winGameFX.play()
    ...
    document.querySelector('.play-again').addEventListener('click', () => {
        ...
        playerTurn = nextTurn()
        newHand()
        if ((playerTurn==1) || (playerTurn==2)) { setTimeout(playCPU, cpuDelay)}
    })
}
//#endregion
...
const playCPU = () => {   
  if (((playerTurn==1) || (playerTurn==2)) && gameOn) {
        if (playerTurn==1) { 
            console.log('cpu beginning turn')
            cpHnd=cpuHand
        }
        else {
            console.log('cpu2 beginning turn')
            cpHnd=cpu2Hand
        }     
        ...
        const playable = determinePlayableCards(cpHnd)
        ...
        //if one playable card
            ...
            // chosenCard = playable[0]
  		    console.log('CPU has only on card to play  ++++++++++++++++++++++++++')
            console.log(playable[0]) // TODO: remove
        ...
        // if more than one playable cards
            ...
            console.log('cpu has', playable.length, 'playable cards')
  		    console.log('CPU will play with  ++++++++++++++++++++++++++')
...
//#region CPU SPECIFIC FUNCTIONS
    function determinePlayableCards(cpHnd) {
        const playableCards = []
        
//        let cpHnd=cpuHand
//        if (playerTurn==2) {cpHnd=cpu2Hand}


        console.log('last card played:') // TODO: remove
        console.log(playPile[playPile.length - 1])
        for (let i = 0; i < cpHnd.length; i++) {
            if (cpHnd[i].color === playPile[playPile.length - 1].color || cpHnd[i].value === playPile[playPile.length - 1].value || cpHnd[i].color === 'any' || playPile[playPile.length - 1].color === 'any') {
                let validCard = cpHnd.splice(i, 1)
    ...
    function playCPUCard(chosenCard) {
        ...
        console.log('animating CPU card')
            ...
            console.log('animating CPU2 card')  
        }
        pickPlayCardSound()
        // debugger
    ...
    function chooseColorAfterWild() {
		...
		console.log('CPU select a new color=',colors[indexOfMax],' ##########')
    }
    //#endregion
}
...
const startGame = () => {
    ...
    if (playerTurn==1) setTimeout(playCPU, cpuDelay)   //PLAYCPU
    if (playerTurn==2) setTimeout(play2CPU, cpuDelay)   //PLAYCPU
